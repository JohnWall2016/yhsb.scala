...
object DateTimeTest extends TestSuite {
  def tests = Tests {
    test("month") {
      ...
      println(s"尚未缴费年限: ${validBounds.mkString(" | ")} | 共计: ${canBuyMonths}个月")
      ...
      println(s"享受补贴年限: ${bonusSpan.mkString(" | ")} | 共计: ${bonusSpan.months}个月")
      println(s"个人缴费年限: ${ownBuySpan.mkString(" | ")} | 共计: ${ownBuySpan.months}个月")
